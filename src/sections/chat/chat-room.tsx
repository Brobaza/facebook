...
import { ChatRoomLinks } from './chat-room-links';
...
export function ChatRoom({ collapseNav, participants, messages, loading }: Props) {
  ...
  const attachments = get(conversation, 'attachments', []);
  const urls = get(conversation, 'links', []);
  ...
  const renderContent = loading ? (
    ...
    <Scrollbar>
      <div>
        ...
        <ChatRoomLinks urls={urls as any} />
      </div>
    </Scrollbar>
  );
